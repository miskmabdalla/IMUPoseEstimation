...
output_root = 'dataset/reduced_dataset/dataset_no_left_arm'
    ...
    if os.path.isdir(input_folder) and folder_name.startswith("trial_"):
        ...
        if os.path.isfile(input_X_csv):
            ...
            try:
                ...
                left_arm_cols = [col for col in df_X.columns if 'left_arm' in col.lower()]
                df_X.drop(columns=left_arm_cols, inplace=True)
                ...
                print(f"  Removed columns: {left_arm_cols}")
